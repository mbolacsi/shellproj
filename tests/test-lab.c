...
void test_cmd_parse_multiple(void) {
     ...
     char **tst = cmd_parse("ps -a -u -x");
     
     TEST_ASSERT_EQUAL_STRING("ps", tst[0]);
     TEST_ASSERT_EQUAL_STRING("-a", tst[1]);
     TEST_ASSERT_EQUAL_STRING("-u", tst[2]);
     TEST_ASSERT_EQUAL_STRING("-x", tst[3]);
     TEST_ASSERT_NULL(tst[4]);
     cmd_free(tst);
 }
 
 void test_cmd_parse_empty(void) {
     char **actual = cmd_parse("");
 
     // Should be null from the empty string
 ...
 void test_cmd_parse_nonexistant(void) {
     // Test to ensure we can still parse a comman even if it doesnt exist
     char **actual = cmd_parse("nonexistent -selfdestruct");
 
     TEST_ASSERT_EQUAL_STRING("nonexistent", actual[0]);
     TEST_ASSERT_EQUAL_STRING("-selfdestruct", actual[1]);
     TEST_ASSERT_NULL(actual[2]);
 ...
 void test_ch_dir_invalid(void) {
     char **cmd = cmd_parse("cd /nonexistent_directory");
 
     // Save the current directory
     char *current_dir = getcwd(NULL, 0);
     
     // Call change_dir with the invalid directory
     change_dir(cmd);
     
     // Store new directory
     char *new_dir = getcwd(NULL, 0);
     
     // Test that the current directory has not changed
     TEST_ASSERT_EQUAL_STRING(current_dir, new_dir);
...
int main(void) {
  ...
  RUN_TEST(test_cmd_parse_nonexistant);
